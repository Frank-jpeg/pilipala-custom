...
TV 版显示名为 `云视听大电视`，提供 `扫码登录`、`手机号登录`、`网页登录兜底` 三种登录路径；在遥控器场景下，手机号点选验证码和网页登录风控页都支持方向键移动光标、`OK` 点击。
TV 首页使用紧凑的两行顶部导航：第一行是 `搜索 / 媒体库 / 登录|我的 / 设置`，第二行是 `推荐 / 历史 / 收藏 / 稍后再看`。`推荐` 保留首页预览与空闲自动全屏，`历史 / 收藏 / 稍后再看` 只展示封面和详情。播放页按遥控器菜单键可调整弹幕开关、速度/时长、字体、透明度、描边、显示区域和屏蔽类型。
TV 设置页顶部显示当前版本和本次/今日观看进度，支持自定义单次与每日时长；可选择开启工作日/休息日分配，默认周一到周五按工作日、周六周日和家长自定义假期按休息日。休息页提供圆形倒计时，可用家长 PIN 或简单/中等/困难算术解锁；算术通过数字键盘输入答案，每次锁定最多答错 3 次，PIN 始终作为备用。验证成功后可选择临时延长 `10 / 15 / 20` 分钟。
